...
class SequentialModel:
    ...
    def forward_prop(self, X, is_training=True):
        ...
        reg_cost_sum = 0
        for layer in self.layers:
            A, reg_cost = layer.layer_forward(A, is_training)
            reg_cost_sum += reg_cost
        return A, reg_cost_sum
    ...
    def compute_cost(self, AL, Y, reg_cost, loss):
        m = Y.shape[1]
    ...
    def fit(self, X, Y, loss='binary_crossentropy', optimizer=None, num_iterations=1000, learning_rate=0.075, print_status=True):
        ...
        for i in range(num_iterations):
            AL, reg_cost = self.forward_prop(X)
            cost = self.compute_cost(AL, Y, reg_cost, loss)
